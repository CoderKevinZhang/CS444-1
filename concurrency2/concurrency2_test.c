...
void get_forks(int x)
{
	...
	/* This is an optimization that checks if a philosopher	*/
	/* would take the forks of the philosopher at the head	*/
	/* of the queue. If not, he is free to go ahead and eat	*/
	else if(Front() != -1 &&
	      state[(x + 1) % 5] != EATING &&
	      state[(x + 4) % 5] != EATING &&
	      (Front() + 1) % 5 != (x + 1) % 5 &&
...
int main()
{
	...
	for(i = 0; i < 5; i++) {
		pthread_cond_init(&pcond[i], NULL);
	}
	for(i = 0; i < 5; i++) {
	...
	while(1) {
		...
		if(j % 5 == 0) {
			...
			printf("Optimization has run %d times\n",
			      opt);
			printf("====================="
			      "=====================\n");
		}
